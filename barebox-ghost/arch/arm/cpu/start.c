		...
		uint32_t boarddata)
{
	...
	printf("%s: before setup C.\n", __func__);
	setup_c();
	printf("%s: after setup C.\n", __func__);
	...
	printf("%s: malloc_start: [%8x], malloc_end: [%8x]\n", __func__, malloc_start, malloc_end);
...
{
	void (*putc)(int ch);
	extern void __puts(char *str);
	arm_setup_stack(membase + memsize - 16);
	__puts("./arch/arm/cpu/start.c\n");
	printf("mem: base[%x], size[%x]\n", membase, memsize);
	putc = (void (*)(int)) boarddata;
	putc('E');
